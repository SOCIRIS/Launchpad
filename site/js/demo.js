...
SOCIRIS.demo = {
  ...
  investigation: function(containerId) {
    ...
    var steps = [
      { icon: '⚡', title: 'Alert Received', desc: 'Brute force SSH attack detected on prod-web-03', state: 'done' },
      { icon: '🔍', title: 'Context Graph Lookup', desc: 'Asset: Crown jewel | Owner: Platform Team | Blast radius: 6 services', state: 'done' },
      { icon: '🧠', title: 'AI Triage — Lightweight Model', desc: 'Verdict: SUSPICIOUS (confidence: 72%) — Escalating to deep analysis', state: 'done' },
      { icon: '🔬', title: 'Deep Analysis — Heavy Model', desc: 'Pattern matches Incident #891. Source IP linked to APT29 infrastructure. 3 other hosts affected.', state: 'active' },
      { icon: '👤', title: 'HITL Verification Gate', desc: 'Awaiting analyst approval for automated response...', state: 'pending' },
      { icon: '🛡️', title: 'Response Execution', desc: 'Block source /24 at firewall + isolate affected hosts', state: 'pending' },
    ];

    function renderSteps() {
      var html = '';
      steps.forEach(function(s) {
        html += '<div class="inv-step">' +
          '<div class="inv-step-icon ' + s.state + '">' + s.icon + '</div>' +
          '<div class="inv-step-content"><h4>' + s.title + '</h4><p>' + s.desc + '</p></div></div>';
      });
    ...
    var id = setInterval(function() {
      ...
      for (var i = 0; i < steps.length; i++) {
        if (steps[i].state === 'active') { activeIdx = i; break; }
      }
      if (activeIdx >= 0) {
